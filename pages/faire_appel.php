...
if (isset($_POST['submit_attendance'])) {
    ...
    $edt_id_submit = (int)$_POST['edt_id'];
    $date_cours = date('Y-m-d');
    $attendances = $_POST['attendance'] ?? [];

    // Récupérer le nom de la matière pour le message
    $stmt_m = $conn->prepare("SELECT m.nom FROM emploi_du_temps edt JOIN matieres m ON edt.matiere_id = m.id WHERE edt.id = ? AND edt.enseignant_id = ?");
    $stmt_m->bind_param("ii", $edt_id_submit, $enseignant_id);
    $stmt_m->execute();
    $m_res = $stmt_m->get_result()->fetch_assoc();
    $stmt_m->close();

    if (!$m_res) {
        $error = "Cours introuvable ou vous n'avez pas les droits sur cette séance.";
    } else {
        $matiere_nom = $m_res['nom'];

        $conn->begin_transaction();
        try {
            $del = $conn->prepare("DELETE FROM presences WHERE emploi_du_temps_id = ? AND date_cours = ?");
            $del->bind_param("is", $edt_id_submit, $date_cours);
            $del->execute();
            $del->close();
