...
$date_debut = (isset($_GET['date_debut']) && preg_match('/^\d{4}-\d{2}-\d{2}$/', $_GET['date_debut'])) ? $_GET['date_debut'] : date('Y-m-01');
$date_fin = (isset($_GET['date_fin']) && preg_match('/^\d{4}-\d{2}-\d{2}$/', $_GET['date_fin'])) ? $_GET['date_fin'] : date('Y-m-d');

// Requête de base pour les statistiques avec requête préparée
$stmt_s = $conn->prepare("
    SELECT 
        COUNT(*) as total,
...
");
$stmt_s->bind_param("ss", $date_debut, $date_fin);
$stmt_s->execute();
